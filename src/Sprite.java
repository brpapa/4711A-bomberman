...
class Sprite {
   final static int RESIZE = 3;
   final static int LIN = 11, COL = 17;
   final static int width = 16 * RESIZE;
   final static int height = 16 * RESIZE;
   final static int qteMaxPlayers = 2;
   
   
   final static Grid g[][] = new Grid[LIN][COL];
   static void setGrid() {
      for (int i = 0; i < LIN; i++)
         for (int j = 0; j < COL; j++)
            g[i][j] = new Grid(width * j, height * i);
   }

   // PARA CADA ID (indice dos vetores)
   final static String personColors[] = {"white", "black", "red", "yellow"};
   final static Grid spawn[] = {g[1][1], g[LIN-2][COL-2], g[LIN-2][1], g[1][COL-2]};


   final static Hashtable<String, Image> ht = new Hashtable<String, Image>();
   final static String mapKeyWords[] = { "background", "block-on-fire-1", "block-on-fire-2", "block-on-fire-3",
